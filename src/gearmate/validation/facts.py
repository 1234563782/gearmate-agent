...
class FactSnapshot:
    ...
    def validate(self, text: str) -> FactValidationResult:
        ...
        stated_amounts = {
            self._money_value(first or second)
            for first, second in MONEY_PATTERN.findall(text)
        }
        unsupported_amounts = tuple(
            sorted(str(amount) for amount in stated_amounts if amount not in allowed_amounts)
        )
        allowed_counts = {
            result.available_count for result in self.availability.values()
        }
    ...
    def fallback_text(self) -> str:
        ...
        for product in list(self.products.values())[:5]:
            ...
            if available is not None:
                line += f"：可租 {available.available_count} 台" if available.available else "：当前租期不可租"
            lines.append(line)
